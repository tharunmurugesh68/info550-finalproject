...
def solve_einstein_puzzle():
    ...
    answers = einstein_puzzle.getSolutionIter()
    ...
    if answers:
        for answer in answers:
            for home in range(1, 6):
                print(f"\nHouse {home}")
                house_is_assigned = False
                for x in criteria:
                    if answer[x] == home:
                        print(x)
                        # Forward checking -> Check if the house has been assigned to any variable
                        for y in criteria:
                            if y != x and answer[y] == home:
                                break
                        else:
                            house_is_assigned = True
                            continue
                        house_is_assigned = True
                        break
                if not house_is_assigned:
                    break
